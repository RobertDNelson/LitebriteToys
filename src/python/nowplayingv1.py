...
f = urllib2.urlopen('http://10.1.3.251/litebrite/peggy/set_color/' + leasecode + '/green')

# Write message to the board
row = 9
col = 0
eightySpaces = " " * 79
prefixes = ["Now Playing: ", "Last Played: ", "Double Prev: "]
prefixIter = prefixes.__iter__()
colorList = ["green", "orange", "red"]
colorIter = colorList.__iter__()
for line in contents:
    f = urllib2.urlopen('http://10.1.3.251/litebrite/peggy/set_color/' + leasecode + '/' + colorIter.next())
    #Write spaces over existing line
    values = {'lease_code':leasecode, 'row':str(row), 'col':str(col), 'msg':eightySpaces}
    data = urllib.urlencode(values)
    f = urllib2.Request('http://10.1.3.251/litebrite/peggy/write', data)
    openResult = urllib2.urlopen(f)
    #Write current line
    values = {'lease_code':leasecode, 'row':str(row), 'col':str(col), 'msg':prefixIter.next() + line}
    data = urllib.urlencode(values)
    f = urllib2.Request('http://10.1.3.251/litebrite/peggy/write', data)
    openResult = urllib2.urlopen(f)
    row = row + 1
...
#Expire our lease code when finished so others can write
f = urllib2.urlopen('http://10.1.3.251/litebrite/peggy/2/expire_lease/' + leasecode)
